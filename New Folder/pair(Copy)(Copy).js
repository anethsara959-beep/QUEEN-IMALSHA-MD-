...
const BOT_NAME_FANCY = '🍁𝗤𝗨𝗘𝗘𝗡 𝗜𝗠𝗔𝗟𝗦𝗛𝗔 𝗠𝗗🍁';
...
const config = {
  ...
  // NEW: default upload limit (MB). null means unlimited
  DEFAULT_UPLOAD_LIMIT_MB: null,
  // NEW: default auto-restart behavior. false -> do NOT auto-restart by default
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // sanitized session number & session config (used for dynamic prefix, upload limit, autoRestart etc.)
    ...
    // helper: download quoted media into buffer (enforces session upload limit if set)
    async function downloadQuotedMedia(quoted) {
      ...
      // enforce upload limit if set in session config (in MB). If uploadLimitMB is null/undefined -> unlimited
      const limitMB = (typeof sessionConfig.uploadLimitMB !== 'undefined') ? sessionConfig.uploadLimitMB : config.DEFAULT_UPLOAD_LIMIT_MB;
      if (limitMB && Number(limitMB) > 0) {
        const bytesLimit = Number(limitMB) * 1024 * 1024;
        if (buffer.length > bytesLimit) {
          try { await socket.sendMessage(sender, { text: `❌ File too large. Allowed upload limit for this session: ${limitMB} MB` }, { quoted: msg }); } catch(e){}
          return null;
        }
      }
    ...
    try {
        ...
        if (!text || text.length < 1) {
            await socket.sendMessage(from, {
                text: cineFormat(
                    '🎬 QUEEN IMALSHA SINHALASUB MOVIE SEARCH',
        ...
        const movieSelectionHandler = async (chatUpdate) => {
            try {
                ...
                let qualityCaption = `*☘️ 𝗧𝗶𝘁𝗹𝗲 ➮* _${result.title || 'N/A'}_\n\n` +
                    ...
                    `*🧚‍♂️𝗥𝗔𝗧𝗜𝗠𝗘➮* _${result.rating || 'N/A'}_\n` +
                    `*⏰𝗥𝗨𝗡𝗧𝗜𝗠𝗘➮* _${result.duration || 'N/A'}_\n` +
                    `*💁‍♂️𝗦𝗨𝗕𝗧𝗜𝗧𝗟𝗘 𝗕𝗬➮* _${result.author || 'N/A'}_\n` +
...
case 'alive': {
  try {
    ...
    const text = `
🤖 *${botName}* is online!
👑 *Owner*: ${config.OWNER_NAME || '𝗢𝗟𝗗 𝗦𝗔𝗡𝗨'}
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '🍁𝗤𝗨𝗘𝗘𝗡 𝗜𝗠𝗔𝗟𝗦𝗛𝗔 𝗠𝗗🍁';
...
│┌───────────
││ 🤖 *Bot Name*: ${title}
││ 👑 *Owner*: ${config.OWNER_NAME || '𝗢𝗟𝗗 𝗦𝗔𝗡𝗨 𝗫'}
...
case 'settings': {
  ...
  try {
    ...
    const botName = userCfg.botName || 'QUEEN IMALSHA MD';
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> 🍁𝗤𝗨𝗘𝗘𝗡 𝗜𝗠𝗔𝗟𝗦𝗛𝗔 𝗠𝗗🍁',
            buttons: buttons,
            headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'grouplist': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || BOT_NAME_FANCY || "QUEEN IMALSHA MD";
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TEL;type=CELL;type=VOICE;waid=94772563976:+94785893445\n' + // WhatsApp Number
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'QUEEN IMALSHA MD'; // dynamic bot name
  ...
  break;
      ...
      case 'getconf': {
        try {
          const sanitized = (number || '').replace(/[^0-9]/g, '');
          const existing = await loadUserConfigFromMongo(sanitized) || {};
          const p = existing.prefix || config.PREFIX;
          const upl = (typeof existing.uploadLimitMB !== 'undefined' && existing.uploadLimitMB !== null) ? `${existing.uploadLimitMB} MB` : 'unlimited';
          const ar = (typeof existing.autoRestart === 'undefined') ? (config.AUTO_RESTART_DEFAULT ? 'enabled (default)' : 'disabled (default)') : (existing.autoRestart ? 'enabled' : 'disabled');
          await socket.sendMessage(sender, { text: `🔧 Session settings:\n• Prefix: ${p}\n• Upload limit: ${upl}\n• Auto-restart: ${ar}` }, { quoted: msg });
        } catch(e){ console.error('getconf error', e); await socket.sendMessage(sender, { text: '❌ Failed to fetch config.' }, { quoted: msg }); }
        break;
      }
  ...
  socket.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (isLoggedOut) {
        ...
        // Check per-session autoRestart setting, default now comes from config.AUTO_RESTART_DEFAULT
        try {
          const sessionConfig = await loadUserConfigFromMongo(number) || {};
          const shouldAutoRestart = (typeof sessionConfig.autoRestart === 'undefined') ? config.AUTO_RESTART_DEFAULT : !!sessionConfig.autoRestart;
          if (!shouldAutoRestart) {
            console.log(`Auto-restart disabled for ${number}. Not reconnecting.`);
            return;
          }
        } catch(e){ console.warn('Failed to load session config for autorestart check', e); }
      ...
      if (connection === 'open') {
        try {
          ...
          const updatedCaption = formatMessage(useBotName,
            `✅\n\n✅ Successfully connected and ACTIVE!\n\n🔢 Number: ${sanitizedNumber}\n🧚‍♂️ 🕒 Connected at: ${getSriLankaTimestamp()}`,
            useBotName
          );
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: 'QUEEN IMALSHA MD BOT', activesession: activeSockets.size });
});
